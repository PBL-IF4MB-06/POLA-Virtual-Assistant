...
class _LoginPageState extends State<LoginPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _onSuccess() async {
    ...
    if (!mounted) return;
    final welcome = _auth.isAdmin
        ? 'Selamat datang, Admin ${_auth.displayName}! Panel Admin ada di Profil.'
        : 'Selamat datang, ${_auth.displayName}!';
  ...
  void _showError(String message) {
    final multiline = message.contains('\n') || message.length > 90;
    if (multiline) {
      showDialog<void>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: const Text('Tidak bisa lanjut'),
