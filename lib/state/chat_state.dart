...
class ChatState extends ChangeNotifier {
  ...
  Future<void> _respondToUser(
    ...
    // Pastikan gambar/rute terisi meski jalur lokal (tanpa backend media).
    final enriched = _mergeCampusMedia(userPromptForSuggestions, reply);
    ...
    convo.messages.add(
      ChatMessage(
        ...
        createdAt: DateTime.now(),
        sources: [
          for (final s in enriched.sources)
            ChatSource(
              title: s.title,
              excerpt: s.excerpt,
              url: s.url,
        ...
        attachments: [
          for (final img in enriched.imageUrls)
            if (img.url.trim().isNotEmpty)
              ChatAttachment(
                type: ChatAttachmentType.image,
                dataUrl: img.url.trim(),
                fileName: img.label.isNotEmpty ? img.label : 'peta_kampus.png',
              ),
        ],
        routes: [
          for (final r in enriched.routes)
            if (r.mapsUrl.trim().isNotEmpty)
              ChatRoute(
                title: r.title,
  ...
  BotReply _mergeCampusMedia(String question, BotReply reply) {
    if (reply.imageUrls.isNotEmpty || reply.routes.isNotEmpty) return reply;
    ...
    return BotReply(
      ...
      imageUrls: [
        for (final img in hit.images) BotImage(url: img.url, label: img.label),
      ],
      routes: [
        for (final r in hit.routes)
          BotRoute(
            title: r.title,
            fromLabel: r.fromLabel,
            toLabel: r.toLabel,
